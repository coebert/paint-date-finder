...
export function useUserLocation(defaultRadius = 25) {
  ...
  const request = useCallback(() => {
    ...
    navigator.geolocation.getCurrentPosition(
      (pos) => {
        ...
        setCoords(next);
        setStatus('granted');
        writeStored({ coords: next, capturedAt: Date.now(), radiusMiles });
      },
      (err) => {
  ...
  const setRadiusMiles = useCallback(
    (n: number) => {
      ...
      if (coords) writeStored({ coords, capturedAt: Date.now(), radiusMiles: n });
    },
    [coords],
  );
  ...
  return { coords, status, error, radiusMiles, setRadiusMiles, request, clear };
}
